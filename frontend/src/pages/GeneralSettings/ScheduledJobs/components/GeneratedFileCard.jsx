...
export default function GeneratedFileCard({ file }) {
  ...
  const isPdf = isPdfFile(file.filename || file.storageFilename);

  const handleOpenPdf = async () => {
    if (!isPdf || opening || !file.storageFilename) return;
    setOpening(true);
    try {
      await openPdfInNewTab(file.storageFilename, StorageFiles.download);
    } catch {
  ...
  return (
    <div className="flex items-center justify-between bg-zinc-800 light:bg-slate-100 rounded-lg p-3">
      <button
        ...
        disabled={!isPdf || opening || !file.storageFilename}
        title={isPdf ? "Open PDF" : undefined}
        className={`flex items-center gap-3 min-w-0 border-none bg-transparent p-0 text-left ${isPdf ? "cursor-pointer hover:opacity-80" : "cursor-default"}`}
      >
        <div
