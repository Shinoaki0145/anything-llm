...
import { isPdfFile, openPdfInNewTab } from "@/utils/pdfPreview.mjs";
...
function FileDownloadCard({ props }) {
  ...
  const isPdf = isPdfFile(filename || storageFilename);

  const handleOpenPdf = async () => {
    if (!isPdf || opening || !storageFilename) return;
    setOpening(true);
    try {
      await openPdfInNewTab(storageFilename, StorageFiles.download);
    } catch {
      ...
      <div className="w-full max-w-[750px] mr-4">
        <div className="flex items-center justify-between bg-zinc-800 light:bg-slate-100 light:border light:border-slate-200/50 rounded-xl px-2 py-1">
          <button
            ...
            disabled={!isPdf || opening || !storageFilename}
            title={isPdf ? "Open PDF" : undefined}
            className={`flex items-center gap-x-3 min-w-0 border-none bg-transparent p-0 text-left ${isPdf ? "cursor-pointer hover:opacity-80" : "cursor-default"}`}
          >
            <div
