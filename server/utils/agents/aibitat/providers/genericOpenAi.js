...
class GenericOpenAiProvider extends InheritMultiple([Provider, UnTooled]) {
  ...
  async #createFunctionCallCompletion(body, disableThinking) {
    if (!disableThinking)
    ...
    try {
      ...
      const message = [error?.message, error?.error?.message]
        .filter(Boolean)
        .join(" ");
      if (!message.includes("chat_template_kwargs")) throw error;
      return await this.client.chat.completions.create(body);
    }
  }

  async #handleFunctionCallChat({ messages = [], disableThinking = false }) {
    return await this.#createFunctionCallCompletion(
      {
        model: this.model,
        temperature: 0,
        messages,
  ...
  async #handleFunctionCallStream({ messages = [], disableThinking = false }) {
    return await this.#createFunctionCallCompletion(
      {
        model: this.model,
        stream: true,
  ...
  async stream(messages, functions = [], eventHandler = null) {
    ...
    if (!useNative) {
      return await UnTooled.prototype.stream.call(
        ...
        eventHandler,
        { disableThinkingForToolSelection: true }
      );
    }
