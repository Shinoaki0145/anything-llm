...
class UnTooled {
  ...
  hasUnansweredFunctionResult(messages = []) {
    const lastAgentMessage = [...messages]
      .reverse()
      .find(({ role }) => ["assistant", "function"].includes(role));
    return lastAgentMessage?.role === "function";
  }
...
${JSON.stringify(def.parameters.properties, null, 4)}\n`;
  ...
  async stream(
    ...
    chatCallback = null,
    eventHandler = null,
    { disableThinkingForToolSelection = false } = {}
  ) {
    this.providerLog("Untooled.stream - will process this chat completion.");
    // eslint-disable-next-line
    try {
      let completion = { content: "" };
      if (functions.length > 0 && !this.hasUnansweredFunctionResult(messages)) {
        const {
          toolCall,
          text,
          uuid: msgUUID,
          ...
          chatCallback,
          eventHandler,
          { disableThinking: disableThinkingForToolSelection }
        );
        ...
        if (text && disableThinkingForToolSelection) {
          eventHandler?.("reportStreamEvent", {
            type: "removeStatusResponse",
            uuid: msgUUID,
  ...
  async complete(
    ...
    try {
      ...
      if (functions.length > 0 && !this.hasUnansweredFunctionResult(messages)) {
        ...
        if (!disableThinkingForToolSelection) completion.content = text;
      }
