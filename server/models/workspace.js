...
const Workspace = {
   ...
   * The command is unnecessary in automatic mode because every message uses
   * the agent runtime, with native tooling or the provider's fallback.
   * @param {Workspace} workspace - The workspace object to check
   * @returns {Promise<boolean>}
   */
  isAgentCommandAvailable: async function (workspace) {
    return workspace.chatMode !== "automatic";
  },
};
